...
(function (mod) {
  ...
  function updateScrollbars (cm, measure) {
    ...
    if (needsH && measure.scrollWidth <= measure.clientWidth + 1 &&
        sWidth > 0 && !measure.hScrollbarTakesSpace) { needsH = false } // (Issue #2562)
  ...
  function updateDisplayIfNeeded (cm, update) {
    ...
    if (!update.force &&
        ...
        countDirtyView(cm) == 0) { return false }
  ...
  function endOperation_finish (op) {
    ...
    if (op.updatedDisplay && webkit) {
      ...
      if (op.barMeasure.scrollWidth > op.barMeasure.clientWidth &&
          ...
          !hScrollbarTakesSpace(cm)) { updateScrollbars(cm) } // (Issue #2562)
    }
  ...
  function readInput (cm) {
    ...
    if (!cm.state.focused || (hasSelection(input) && !prevInput) || isReadOnly(cm) || cm.options.disableInput) { return false }
    ...
    if (cm.state.pasteIncoming && doc.sel.ranges.length > 1) {
      if (lastCopied && lastCopied.join('\n') == inserted) { multiPaste = doc.sel.ranges.length % lastCopied.length == 0 && map(lastCopied, splitLines) } else if (textLines.length == doc.sel.ranges.length) { multiPaste = map(textLines, function (l) { return [l] }) }
    }
  ...
  function leftButtonDown (cm, e, start) {
    ...
    if (cm.options.dragDrop && dragAndDrop && !isReadOnly(cm) &&
        type == 'single' && sel.contains(start) > -1 && sel.somethingSelected()) { leftButtonStartDrag(cm, e, start, modifier) } else { leftButtonSelect(cm, e, start, type, modifier) }
  }
  ...
  function leftButtonStartDrag (cm, e, start, modifier) {
    ...
    var dragEnd = operation(cm, function (e2) {
      ...
      if (Math.abs(e.clientX - e2.clientX) + Math.abs(e.clientY - e2.clientY) < 10) {
        ...
        if (ie && ie_version == 9) { setTimeout(function () { document.body.focus(); focusInput(cm) }, 20) }
      }
    })
    ...
    function rehide () {
      ...
      if (display.input.selectionStart != null) {
        ...
        var i = 0, poll = function () {
          if (display.selForContextMenu == cm.doc.sel && display.input.selectionStart == 0) { operation(cm, commands.selectAll)(cm) } else if (i++ < 10) display.detectingSelectAll = setTimeout(poll, 500)
          else resetInput(cm)
        }
  ...
  var bidiOrdering = (function () {
    ...
    function charType (code) {
      ...
      else if (code >= 0x600 && code <= 0x6ed) return arabicTypes.charAt(code - 0x600)
