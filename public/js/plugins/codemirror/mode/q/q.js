...
(function (mod) {
  ...
  CodeMirror.defineMode('q', function (config) {
    ...
    function tokenBase (stream, state) {
      ...
      curPunc = null
      if (sol) {
        if (c == '/') { return (state.tokenize = tokenLineComment)(stream, state) } else if (c == '\\') {
          if (stream.eol() || /\s/.test(stream.peek())) { return stream.skipToEnd(), /^\\\s*$/.test(stream.current()) ? (state.tokenize = tokenCommentToEOF)(stream, state) : state.tokenize = tokenBase, 'comment' } else { return state.tokenize = tokenBase, 'builtin' }
        }
      }
      ...
      stream.match(/^\d+[ptuv]{1}/)) { t = 'temporal' } else if (stream.match(/^0[NwW]{1}/) ||
      stream.match(/^0x[\d|a-f|A-F]*/) ||
      stream.match(/^[0|1]+[b]{1}/) ||
      stream.match(/^\d+[chijn]{1}/) ||
      stream.match(/-?\d*(\.\d*)?(e[+\-]?\d+)?(e|f)?/)) { t = 'number' }
        return (t && (!(c = stream.peek()) || E.test(c))) ? t : (stream.next(), 'error')
      }
      if (/[A-Z|a-z]|\./.test(c)) { return stream.eatWhile(/[A-Z|a-z|\.|_|\d]/), keywords.test(stream.current()) ? 'keyword' : 'variable' }
