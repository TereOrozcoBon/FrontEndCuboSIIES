...
(function (window, $) {
  ...
  var ngSearchProvider = function ($scope, grid, $filter) {
    ...
    var evalObject = function (obj, columnName) {
      if (typeof obj !== 'object' || typeof columnName !== 'string') { return obj }
    ...
    if (!self.extFilter) {
      $scope.$watch('columns', function (cs) {
        for (var i = 0; i < cs.length; i++) {
          var col = cs[i]
          if (col.field) { self.fieldMap[col.field.split('.')[0]] = col }
          if (col.displayName) { self.fieldMap[col.displayName.toLowerCase().replace(/\s+/g, '')] = col }
        };
      })
    }
  }
            ...
            options.selectRow = function (rowIndex, state) {
              if (grid.rowCache[rowIndex]) {
                if (grid.rowCache[rowIndex].clone) {
                  grid.rowCache[rowIndex].clone.setSelection(!!state)
                }
                grid.rowCache[rowIndex].setSelection(!!state)
              }
            }
