  ...
  angular.scenario.dsl = angular.scenario.dsl || function (name, fn) {
    angular.scenario.dsl[name] = function () {
      function executeStatement (statement, args) {
        ...
        if (angular.isFunction(result) || result instanceof angular.scenario.Future) { return result }
        var self = this
 ...
 */
  angular.scenario.Application.prototype.getWindow_ = function () {
    ...
    if (!contentWindow) { throw 'Frame window is not accessible.' }
 ...
 */
  angular.scenario.ObjectModel = function (runner) {
    ...
    runner.on('StepEnd', function (spec) {
      ...
      if (step.name !== step.name) { throw 'Events fired in the wrong order. Step names don\'t match.' }
      complete(step)
 ...
 */
  angular.scenario.Runner.prototype.createSpecRunner_ = function (scope) {
    ...
    for (var name in Cls.prototype) { child[name] = angular.bind(child, Cls.prototype[name]) }
  ...
  angular.scenario.dsl('repeater', function () {
    ...
    chain.row = function (index) {
      return this.addFutureAction("repeater '" + this.label + "' row '" + index + "'", function ($window, $document, done) {
        ...
        if (!matches.length) { return done('row ' + index + ' out of bounds') }
