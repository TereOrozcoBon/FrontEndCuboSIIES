  ...
  $.fn.justifiedGallery = function (options) {
    ...
    return this.each(function (index, cont) {
      ...
      $(cont).find('a').each(function (index, entry) {
        ...
        $(img).load(function () {
          if (images[index]['height'] != settings.rowHeight) { images[index]['width'] = Math.ceil(this.width / (this.height / settings.rowHeight)) } else { images[index]['width'] = this.width }
    ...
    function buildImage (image, suffix, nw, nh, l, minRowHeight, settings) {
      ...
      if (settings.captions) { ris += '  <div style="bottom:' + (nh - minRowHeight) + 'px;" class="jg-image-label">' + image['alt'] + '</div>' }
    ...
    function buildContRow (row, images, extraW, settings) {
      ...
      for (var j = 0; j < row.length; j++) {
        ...
        if (!settings.fixedHeight) {
          if (j == 0) { minRowHeight = row[j]['nh'] } else
